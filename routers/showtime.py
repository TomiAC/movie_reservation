...
@showtime_router.get("/movie/history/{movie_name}")
async def get_movie_showtimes_history(movie_name: str, db: Session = Depends(get_db), skip: int = 0, limit: int = 100, current_user: str = Depends(has_role("admin"))):
    return await get_movie_active_showtimes(db, movie_name, skip, limit)

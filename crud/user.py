...
async def update_password(db: Session, new_password: str, user_email: UserEmail):
    user = db.query(User).filter(user_email.email==User.email).first()
...
async def delete_user(db: Session, user_email: UserEmail):
    user = db.query(User).filter(user_email.email==User.email).first()
    db.delete(user)
    db.commit()
    return user

async def update_email(db: Session, new_email: str, user_email: UserEmail):
    user = db.query(User).filter(user_email.email==User.email).first()
